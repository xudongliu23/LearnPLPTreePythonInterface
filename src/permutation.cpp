...
void generateBinaries(vstring *binaries, vstring *have, int number_of_bits_total)
{
    // base case
    if( (int) (*have)[0].size() == number_of_bits_total )
    {
        binaries->push_back((*have)[0]);
//        cout << (*have)[0] << endl;
        return;
    }
    ...
    for (int i=0; i<2; i++)
    {
        string cp_have = (*have)[0];
				vstring vs;
				vs.push_back(cp_have.append(numberToString(i)));

        generateBinaries(binaries, &vs, number_of_bits_total);
...
string getNextLexPerm(string this_perm)
{
    ...
    else
    {
        ...
        /* 3. Reverse this_perm[pos+1..length-1] */
				vstring vs;
				vs.push_back(this_perm);
        reverseString(&vs, pos+1, length-1);

        return this_perm;
    }
}

void reverseString(vstring *str, size_t start, size_t end)
{
    for(size_t i=start; i<=(start+end)/2; i++)
    {
        // switch
        char tmp = ((*str)[0])[i];
        ((*str)[0])[i] = ((*str)[0])[end-i+start];
        ((*str)[0])[end-i+start] = tmp;
    }
}
